...
Now I need to add the `sendQueryResponse` function to the `whatsappNotifications.ts` file:

<write file="utils/whatsappNotifications.ts">
import { getSupabase } from '@/lib/supabase';
...
/**
 * Sends query response to customer via WhatsApp
 */
export async function sendQueryResponse(
  ...
  try {
    ...
    if (!config || !config.access_token || !config.phone_number_id) {
      console.log('WhatsApp not configured, cannot send response');
